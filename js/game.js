...
function drawGameWorld() {
    ...
    skyGradient.addColorStop(0.55, "#263746");
    ...
    gameContext.arc(gameWidth * 0.82, gameHeight * 0.16, 42, 0, Math.PI * 2);
    gameContext.fill();
    /* Земля. */
    const groundHeight = gameHeight * 0.30;
    gameContext.fillStyle = "#252a28";
    gameContext.fillRect(0, gameHeight - groundHeight, gameWidth, groundHeight);
    /* Пиксельная сетка земли. */
    gameContext.strokeStyle = "rgba(0, 0, 0, 0.16)";
    gameContext.lineWidth = 2;
    const blockSize = 32;
    for (let x = 0; x < gameWidth; x += blockSize) {
        for (let y = gameHeight - groundHeight; y < gameHeight; y += blockSize) {
