...
const Settlement = {
    ...
    draw(ctx,width,height) {
        ...
        this.drawGround(ctx,width,height,centerX,centerY);
        this.buildings.forEach((building) => this.drawBuilding(ctx,building,centerX,centerY));
    },
    drawGround(ctx,width,height,centerX,centerY) {
        const size = Math.min(width * 0.82,700);
        ctx.fillStyle = "#1b2020";
        ctx.fillRect(centerX - size / 2,centerY - 40,size,size * 0.62);
